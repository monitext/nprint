...
export function box(str: string, { color = "gray", rounded = false }: BoxOptions = {}): string {
  ...
  const body = lines.map(line => `${style(chars.v)} ${line.padEnd(width)} ${style(chars.v)}`).join("\n")
...
console.log(
    ...render(box(`text string`, { color: "blue", rounded: true }))
);